...
fileprivate let subsystem = PortLogger.makeSubsystem("fgroup")
...
class FileInfo: Hashable {
  ...
  init(_ url: URL) {
    self.url = url
    self.path = url.path
    self.ext = UtilityURLNormalizedExtension.lowercasedExtension(of: url)
...
class FileGroup {
  ...
  static func group(files: [FileInfo]) -> FileGroup {
    PortLogger.log("Start grouping \(files.count) files", subsystem: subsystem)
  ...
  private func tryGroupFiles() {
    PortLogger.log("Try group files, prefix=\(prefix), count=\(contents.count)", level: .verbose, subsystem: subsystem)
    guard contents.count >= 3 else {
      PortLogger.log("Contents count < 3, skipped", level: .verbose, subsystem: subsystem)
      return
    }
    ...
    if stopGrouping(currChars) || maxSubGroupCount < 3 {
      PortLogger.log("Stop grouping, maxSubGroup=\(maxSubGroupCount)", level: .verbose, subsystem: subsystem)
      contents.forEach { $0.prefix = self.prefix }
    } else {
      PortLogger.log("Continue grouping, groups=\(tempGroup.count), chars=\(currChars)", level: .verbose, subsystem: subsystem)
