...
fileprivate extension String {
  /// Wraps the string in mpv fixed-length quoting: `%N%string`
  var mpvFixedLengthQuoted: String { "%\(count)%\(self)" }
 ...
 */
final class MPVFilter: @unchecked Sendable, Equatable {
  ...
  convenience init(lavfiFilterFromPresetInstance instance: FilterPresetInstance) {
    self.init(lavfiName: instance.preset.name,
              label: nil,
              paramDict: instance.params.mapValues { $0.stringValue })
  }

  convenience init(mpvFilterFromPresetInstance instance: FilterPresetInstance) {
    let paramString = instance.preset.paramOrder.map {
      instance.value(for: $0).stringValue
    }.joined(separator: ":")
    self.init(name: instance.preset.name,
              label: nil,
  ...
  private static func parseRawParamString(_ name: String, _ rawParamString: String?) -> [String: String]? {
    ...
    for pair in pairs {
      ...
      guard split.count == 2 else {
        ...
        // kind of filter.
        PortLogger.log("Unable to parse filter \(name) params: \(rawParamString)", level: .warning)
        return nil
      }
  ...
  static func == (lhs: MPVFilter, rhs: MPVFilter) -> Bool {
    guard lhs.label == rhs.label, lhs.name == rhs.name else { return false }
    if let l = lhs.params, let r = rhs.params { return l == r }
