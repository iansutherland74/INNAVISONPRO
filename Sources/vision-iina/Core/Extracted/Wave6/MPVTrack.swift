...
final class MPVTrack: @unchecked Sendable {
  ...
  var infoString: String {
    get {
      ...
      // lang
      let language: String
      if let lang, lang != "und" {
        let normalizedLanguage = ISO639HelperCore.languageName(for: lang)
        let rawLanguage = ISO639Helper.dictionary[lang]
        if let normalizedLanguage {
          language = "[\(normalizedLanguage)]"
        } else if let rawLanguage {
