...
struct Sysctl {
  ...
  private func getString(_ name: String) -> String? {
    ...
    guard sysctlbyname(name, nil, &size, nil, 0) == EXIT_SUCCESS else {
      PortLogger.log("Call to sysctlbyname for \(name) failed: \(String(cString: strerror(errno))) (\(errno))", level: .warning)
      return nil
    }
    ...
    return value.withUnsafeBytes { String(decoding: $0.prefix(while: { $0 != 0 }), as: UTF8.self) }
  }
}
